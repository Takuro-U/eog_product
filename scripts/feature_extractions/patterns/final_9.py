...
オフセット設計方針:
    ...
    range, std, slope, skew, iqr はいずれも定数オフセット非依存である。
...
def _compute_features(channels: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    features["iqr_h"] = float(np.percentile(ch1, 75) - np.percentile(ch1, 25))
    features["iqr_v"] = float(np.percentile(ch3, 75) - np.percentile(ch3, 25))
