...
特徴量カテゴリ:
    ...
    3. サッカード活動度・方向・局在 (10次元): 活動強度の局在性・方向性・タイミング
...
縦横スケール設計方針:
    ...
    （rms_diff / std_diff）を基底として比率を計算する。
    これにより縦横スケール差を吸収する。
"""
...
FEATURE_COLUMNS: list[str] = [
    ...
    # 2.4 統合・関係特徴 (2)
    # norm = rms_diff / (std_diff + EPSILON) としてチャンネル内正規化した上での縦横比率
...
def _compute_features(channels: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    # 前半・後半スライス（差分系列）
    diff_sac_h_first = diff_sac_h[:half_diff]
    diff_sac_h_last = diff_sac_h[half_diff:]
    diff_sac_v_first = diff_sac_v[:half_diff]
    diff_sac_v_last = diff_sac_v[half_diff:]
    ...
    # 前半・後半の活動強度（局在性）
    features["rms_diff_sac_h_first"] = float(np.sqrt(np.mean(diff_sac_h_first ** 2)))
    features["rms_diff_sac_h_last"] = float(np.sqrt(np.mean(diff_sac_h_last ** 2)))
    features["rms_diff_sac_v_first"] = float(np.sqrt(np.mean(diff_sac_v_first ** 2)))
    features["rms_diff_sac_v_last"] = float(np.sqrt(np.mean(diff_sac_v_last ** 2)))
