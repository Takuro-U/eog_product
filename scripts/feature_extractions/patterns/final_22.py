...
スパイクマスク設計方針:
    ...
    閾値未満の差分サンプルを0とみなしたスパイク系列を生成する。
    サッカード関連の全特徴量はこのスパイク系列から計算する。
"""
...
def _compute_features(channels: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    # スパイクマスク処理
    # 閾値未満の差分サンプルを0に置き換えることで
    # 微細な揺れを除去し、真のサッカード成分のみを残す
    spike_sac_h = _apply_spike_mask(diff_sac_h)
    spike_sac_v = _apply_spike_mask(diff_sac_v)
    ...
    # スパイクマスク済み差分系列から計算
    # ---------------------------------------------------------
    rms_spike_sac_h = float(np.sqrt(np.mean(spike_sac_h ** 2)))
    rms_spike_sac_v = float(np.sqrt(np.mean(spike_sac_v ** 2)))

    features["rms_spike_sac_h"] = rms_spike_sac_h
    features["rms_spike_sac_v"] = rms_spike_sac_v

    # 前半・後半の平均スパイク差分（方向性 + 局在性）
    features["mean_spike_sac_h_first"] = float(np.mean(spike_sac_h[:half_diff]))
    features["mean_spike_sac_h_last"] = float(np.mean(spike_sac_h[half_diff:]))
    features["mean_spike_sac_v_first"] = float(np.mean(spike_sac_v[:half_diff]))
    features["mean_spike_sac_v_last"] = float(np.mean(spike_sac_v[half_diff:]))
    ...
    # ---------------------------------------------------------
    std_spike_sac_h = float(np.std(spike_sac_h))
    std_spike_sac_v = float(np.std(spike_sac_v))

    norm_h = rms_spike_sac_h / (std_spike_sac_h + EPSILON)
    norm_v = rms_spike_sac_v / (std_spike_sac_v + EPSILON)
...
def _apply_spike_mask(diff: np.ndarray) -> np.ndarray:
    """
    差分系列にスパイクマスクを適用する

    閾値（標準偏差 × SACCADE_SPIKE_THRESHOLD_STD）未満の
    絶対値を持つサンプルを0に置き換える。
    これにより微細な揺れを除去し、真のサッカード成分のみを残す。

    Args:
        diff: 一階差分系列

    Returns:
        スパイクマスク済み系列（閾値未満のサンプルは0）
    """
    threshold = np.std(diff) * SACCADE_SPIKE_THRESHOLD_STD
    masked = diff.copy()
    masked[np.abs(masked) < threshold] = 0.0
    return masked
