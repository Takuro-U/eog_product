...
特徴量カテゴリ:
    ...
    2. 位置トレンド・変化量 (8次元): 移動中の検出・タイミング・非線形成分
...
オフセット設計方針:
    ...
    range, std, slope, skew, residual_std はいずれも定数オフセット非依存である。
...
FEATURE_COLUMNS: list[str] = [
    ...
    "slope_v_first",
    "slope_v_last",
    "residual_std_h",  # 線形回帰残差の標準偏差（非線形成分の大きさ）
    "residual_std_v",
...
def _compute_features(channels: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    features["slope_h"] = slope_h
    features["slope_v"] = slope_v
    ...
    # 線形回帰残差の標準偏差（非線形成分の大きさ）
    # 残差 = 実測値 - 線形回帰値
    t_mean = float(np.mean(t))
    features["residual_std_h"] = _compute_residual_std(t, ch1, t_mean, slope_h)
    features["residual_std_v"] = _compute_residual_std(t, ch3, t_mean, slope_v)
...
def _compute_residual_std(
    ...
    残差 = y - (slope * (t - t_mean) + y_mean)
    位置成分の非線形変動成分の大きさを表す。
    定数オフセットは残差計算において相殺される。
    """
    y_mean = np.mean(y)
    y_hat = slope * (t - t_mean) + y_mean
    residuals = y - y_hat
