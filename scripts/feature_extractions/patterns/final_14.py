...
特徴量カテゴリ:
    ...
    4. 統合・関係特徴 (4次元): サッカード軸方向の相対的優勢・縦横相関
...
縦横スケール設計方針:
    ...
    （rms_diff / std_diff）を基底として比率を計算し、スケール差を吸収する。
    corr_pos_hv および corr_diff_sac_hv はピアソン相関係数であり、
    スケール差の影響を受けない。
"""
...
FEATURE_COLUMNS: list[str] = [
    ...
    # 2.4 統合・関係特徴 (4)
    # sac_relative_ratio: チャンネル内正規化済みの軸方向相対優勢
    # corr_*: ピアソン相関係数（スケール差非依存）
...
def _compute_features(channels: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    # チャンネル内正規化済みの軸方向相対優勢
    # norm = rms_diff / (std_diff + EPSILON) として縦横スケール差を吸収
    ...
    # 位置成分の縦横ピアソン相関係数
    # 縦横が連動して動いているかを示す（スケール差非依存）
    features["corr_pos_hv"] = _compute_correlation(ch1, ch3)

    # サッカード差分系列の縦横ピアソン相関係数
    # サッカード発生時の縦横同時性を示す（スケール差非依存）
    features["corr_diff_sac_hv"] = _compute_correlation(diff_sac_h, diff_sac_v)
