...
特徴量カテゴリ:
    ...
    2. 位置トレンド・変化量 (2次元): 移動中の検出・保持中ドリフトの補強
...
sixth_sample.py（18次元）からの追加:
    - skew_h, skew_v : 位置分布の歪度（保持中の非対称な偏りの表現）
"""
...
def _compute_features(channels: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    # 前半・後半の平均差分（方向性 + 局在性）
    features["mean_diff_sac_h_first"] = float(np.mean(diff_sac_h[:half]))
    features["mean_diff_sac_h_last"] = float(np.mean(diff_sac_h[half:]))
    features["mean_diff_sac_v_first"] = float(np.mean(diff_sac_v[:half]))
    features["mean_diff_sac_v_last"] = float(np.mean(diff_sac_v[half:]))
