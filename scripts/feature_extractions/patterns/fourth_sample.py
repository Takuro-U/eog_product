...
特徴量カテゴリ:
    ...
    2. 位置トレンド・変化量 (4次元): 保持中の視線ドリフト・振れ幅の補強

設計方針:
    保持位置（上下左右中央）の識別精度を最大化する構成。
    third_sample.py（6次元）では位置の誤分類が多発したため、
    振れ幅（range）と傾き（slope）を追加し位置表現を補強した。
    移動中クラスの識別精度低下は許容している。

third_sample.py（6次元）からの追加:
    - range_h, range_v : ウィンドウ内の振れ幅（保持安定性の補強）
    - slope_h, slope_v : 位置ドリフトの方向性（保持クラス間境界の補強）
"""
