...
sac_sharpness_ratio 設計方針:
    縦横の振幅スケールが揃っていないため、rms_diff の直接比較は
    スケール差を反映したものになり物理的意味が不正確になる。
    そこでスパイクの急峻さをチャンネル内で正規化した量を基底とする。

        sharpness = std(diff(ch)) / (std(ch) + EPSILON)

    これはチャンネル自身の振幅スケールで正規化されるため、
    縦横スケール差に依存しないサッカード急峻さの比較が可能になる。
"""
...
FEATURE_COLUMNS: list[str] = [
    ...
    # 2.4 統合・関係特徴 (2)
    "sac_sharpness_ratio_h",
    "sac_sharpness_ratio_v",
]
...
def _compute_features(channels: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    # ---------------------------------------------------------
    # 2.4 統合・関係特徴 (2次元)
    # スパイクの急峻さをチャンネル内スケールで正規化した比率
    # sharpness = std(diff(ch)) / (std(ch) + EPSILON)
    # 縦横の振幅スケール差に依存しないサッカード急峻さの比較
    # ---------------------------------------------------------
    sharpness_h = float(np.std(diff_sac_h) / (np.std(ch2) + EPSILON))
    sharpness_v = float(np.std(diff_sac_v) / (np.std(ch4) + EPSILON))
    sharpness_total = sharpness_h + sharpness_v + EPSILON

    features["sac_sharpness_ratio_h"] = sharpness_h / sharpness_total
    features["sac_sharpness_ratio_v"] = sharpness_v / sharpness_total
