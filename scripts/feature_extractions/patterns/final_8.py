...
オフセット設計方針:
    ...
    q25, q75 はオフセットを含むが、教師・タスク間の一致により有効である。
...
seventh_sample.py（24次元）からの追加:
    - q25_h, q75_h, q25_v, q75_v : 四分位数による分布形状の補強
"""
...
def _compute_features(channels: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    features["q25_h"] = float(np.percentile(ch1, 25))
    features["q75_h"] = float(np.percentile(ch1, 75))
    features["q25_v"] = float(np.percentile(ch3, 25))
    features["q75_v"] = float(np.percentile(ch3, 75))
