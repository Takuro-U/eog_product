...
seventh_sample.py（24次元）からの除外:
    - slope_h, slope_v          : slope_*_first/last との冗長性
    - sac_energy_ratio_h/v      : rms_diff_sac_h/v の線形結合であり独立情報なし
"""
...
def _compute_features(channels: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    # ---------------------------------------------------------
    features["rms_diff_sac_h"] = float(np.sqrt(np.mean(diff_sac_h ** 2)))
    features["rms_diff_sac_v"] = float(np.sqrt(np.mean(diff_sac_v ** 2)))
