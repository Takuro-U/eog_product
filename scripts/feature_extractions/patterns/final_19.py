...
特徴量カテゴリ:
    1. 位置レベル・分布 (18次元): 保持位置判別の主軸 + 局在性
    2. 位置トレンド・変化量 (6次元): 移動中の検出・タイミング
    3. サッカード活動度・方向・局在 (10次元): 移動中の識別・方向性・タイミング
    4. 統合・関係特徴 (6次元): サッカードの軸方向相対優勢 + 局在性
...
縦横スケール設計方針:
    ...
    sac_relative_ratio_* は各チャンネルの差分系列内での相対的な傾斜
    （rms_diff / std_diff）を基底として比率を計算することで
    縦横スケール差を吸収する。
"""
...
FEATURE_COLUMNS: list[str] = [
    ...
    "slope_v_first",
    "slope_v_last",
    # 2.3 サッカード活動度・方向・局在 (10)
    "rms_diff_sac_h",
    "rms_diff_sac_v",
    "rms_diff_sac_h_first",
    "rms_diff_sac_h_last",
    "rms_diff_sac_v_first",
    "rms_diff_sac_v_last",
    ...
    "sac_relative_ratio_h_first",
    "sac_relative_ratio_h_last",
    "sac_relative_ratio_v_first",
    "sac_relative_ratio_v_last",
]
...
def _compute_features(channels: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    # 位置成分・差分系列の前半・後半スライス
    ch1_first, ch1_last = ch1[:half_pos], ch1[half_pos:]
    ch3_first, ch3_last = ch3[:half_pos], ch3[half_pos:]
    t_first, t_last = t[:half_pos], t[half_pos:]
    diff_sac_h_first, diff_sac_h_last = diff_sac_h[:half_diff], diff_sac_h[half_diff:]
    diff_sac_v_first, diff_sac_v_last = diff_sac_v[:half_diff], diff_sac_v[half_diff:]
    ...
    # 前半・後半の平均・標準偏差（位置分布の局在性）
    features["mean_h_first"] = float(np.mean(ch1_first))
    features["mean_h_last"] = float(np.mean(ch1_last))
    features["mean_v_first"] = float(np.mean(ch3_first))
    features["mean_v_last"] = float(np.mean(ch3_last))
    features["std_h_first"] = float(np.std(ch1_first))
    features["std_h_last"] = float(np.std(ch1_last))
    features["std_v_first"] = float(np.std(ch3_first))
    features["std_v_last"] = float(np.std(ch3_last))
    ...
    features["slope_h_first"] = _compute_slope(t_first, ch1_first)
    features["slope_h_last"] = _compute_slope(t_last, ch1_last)
    features["slope_v_first"] = _compute_slope(t_first, ch3_first)
    features["slope_v_last"] = _compute_slope(t_last, ch3_last)

    # ---------------------------------------------------------
    # 2.3 サッカード活動度・方向・局在 (10次元)
    # 全て差分系列から計算（定数オフセット非依存）
    # ---------------------------------------------------------
    rms_diff_sac_h = float(np.sqrt(np.mean(diff_sac_h ** 2)))
    rms_diff_sac_v = float(np.sqrt(np.mean(diff_sac_v ** 2)))
    rms_diff_sac_h_first = float(np.sqrt(np.mean(diff_sac_h_first ** 2)))
    rms_diff_sac_h_last = float(np.sqrt(np.mean(diff_sac_h_last ** 2)))
    rms_diff_sac_v_first = float(np.sqrt(np.mean(diff_sac_v_first ** 2)))
    rms_diff_sac_v_last = float(np.sqrt(np.mean(diff_sac_v_last ** 2)))

    features["rms_diff_sac_h"] = rms_diff_sac_h
    features["rms_diff_sac_v"] = rms_diff_sac_v
    features["rms_diff_sac_h_first"] = rms_diff_sac_h_first
    features["rms_diff_sac_h_last"] = rms_diff_sac_h_last
    features["rms_diff_sac_v_first"] = rms_diff_sac_v_first
    features["rms_diff_sac_v_last"] = rms_diff_sac_v_last

    features["mean_diff_sac_h_first"] = float(np.mean(diff_sac_h_first))
    features["mean_diff_sac_h_last"] = float(np.mean(diff_sac_h_last))
    features["mean_diff_sac_v_first"] = float(np.mean(diff_sac_v_first))
    features["mean_diff_sac_v_last"] = float(np.mean(diff_sac_v_last))

    # ---------------------------------------------------------
    # 2.4 統合・関係特徴 (6次元)
    # チャンネル内正規化: norm = rms_diff / (std_diff + EPSILON)
    # 縦横スケール差を吸収した上での軸方向相対優勢を表現する
    # ---------------------------------------------------------
    def _sac_relative_ratio(rms_h: float, rms_v: float,
                             diff_h: np.ndarray, diff_v: np.ndarray
                             ) -> tuple[float, float]:
        norm_h = rms_h / (float(np.std(diff_h)) + EPSILON)
        norm_v = rms_v / (float(np.std(diff_v)) + EPSILON)
        total = norm_h + norm_v + EPSILON
        return norm_h / total, norm_v / total

    ratio_h, ratio_v = _sac_relative_ratio(
        rms_diff_sac_h, rms_diff_sac_v, diff_sac_h, diff_sac_v
    )
    ratio_h_first, ratio_v_first = _sac_relative_ratio(
        rms_diff_sac_h_first, rms_diff_sac_v_first,
        diff_sac_h_first, diff_sac_v_first
    )
    ratio_h_last, ratio_v_last = _sac_relative_ratio(
        rms_diff_sac_h_last, rms_diff_sac_v_last,
        diff_sac_h_last, diff_sac_v_last
    )

    features["sac_relative_ratio_h"] = ratio_h
    features["sac_relative_ratio_v"] = ratio_v
    features["sac_relative_ratio_h_first"] = ratio_h_first
    features["sac_relative_ratio_h_last"] = ratio_h_last
    features["sac_relative_ratio_v_first"] = ratio_v_first
    features["sac_relative_ratio_v_last"] = ratio_v_last
